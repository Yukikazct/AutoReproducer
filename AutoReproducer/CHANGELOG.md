...
## 4. 代码执行
- 代码长度: 0 字符
- 执行状态: ⚠️ 未运行（代码未通过执行前检查）
- 未运行原因: 论文信息不足（缺少方法/数据集/指标），无法生成针对性复现代码
  - precheck: ❌ 失败 (退出码 -5)
```

诉求原话：**「不能这样啊 反正 一定要尝试生成代码并允许啊」**。

**两个根因**：

1. `code_executor.run()` 里 `_info_insufficient(paper_info)` 命中即
   `return _not_runnable(..., code="")` —— 代码 0 字符、stage=precheck、
   exit_code=-5 全出自这一行；
2. 生成 prompt 第 5 条明令模型"信息不足就只输出一行 `# INSUFFICIENT_INFO`
   并停止"，而**代码层从没有任何地方识别这个标记**：它会被判"结构不完整"
   → 掉进续写(3)+重生成(2) 循环，最多 6 次调用全部空烧，最后把一行注释当
   代码送进执行。

**改法**（信息不足从**门**降级为**标注**）：

- **`src/agents/code_executor.py`**：删掉预拒分支，改为照常生成、照常进沙箱；
  新增 `best_effort` / `best_effort_reason` / `fallback_used` / `assumptions`
  四个 additive 字段，随结果一路带到报告。
  - prompt 第 5 条按信息是否充足分叉（新增 `_rule5`）：信息充足时**逐字保持
    原样**（`TestPrompts` 锁死的那几条串全靠这个默认值）；不足时换成"**必须**
    交出最小可运行脚本"——用合成数据/默认值、以 `# 假设: <内容>` 注明来源、
    开头声明"不是论文结论"，并且**保留**禁止把无关数据集/编造数值冒充论文
    结论的禁令（宾语从"生成代码"改成"冒充结论"）。
  - 模型"拒答"（只回占位标记或空）的处置：`_is_placeholder_code` 识别 →
    `MAX_MARK_RETRY=1` 次**定向重试**（换指令问一次，不是同 prompt 重放）→
    仍拒答则落本地兜底脚本 `_BEST_EFFORT_SCRIPT`。预算上限 2 次调用，比旧
    路径的 6 次空烧更省。兜底脚本三条硬约束：纯标准库且不含 `_DANGEROUS_PATTERNS`
    的任何词（连注释都不能有）、**打印内容抽不出任何指标**（否则会喂出假
    复现结论，这一条有专门用例锁死）、`_structurally_complete` 为真。
- **`src/agents/result_validator.py`**：新增第四态 `best_effort`。不拦的话，
  占位脚本一旦跑出任何可提取数值，`_local_compare` 对"无论文声明指标"的乐观
  判定会把它翻成**假 ✅ 成功**（还会真去触发优化）；一个数值都抽不到时又翻成
  **假 ❌ 失败**。现在：`is_reproduced=None`、跳过 LLM 比对（省预算）、
  指标对照表照常显示"确实跑出了什么"。优先级 `not_runnable` > `best_effort`
  > 正常比对。
- **`src/agents/report_generator.py`**：§4 执行状态标注「（尽力而为：论文信息
  不足）」并补信息不足说明、生成方式（模型生成/系统兜底）与逐条假设；§5 新增
  「⚠️ 无法核对（论文信息不足，代码为占位实现，非论文结论）」（必须排在
  `is_reproduced` 判断之前，否则 None 会被判成 ❌ 失败）。「未运行」字样只留给
  语法错误/危险调用两道**真门**。顺带去掉一处重复：`reason` 已包含在
  `analysis` 里时不再单列「判定原因」。
- **`src/orchestrator.py`** / **`frontend/backend_pipeline.py`**：跳过优化的原因
  新增一条——「代码为尽力而为的占位实现（论文信息不足），无法作为优化基线」。

**真实链路实测**（executor 真执行 → validator → report，论文信息按用户那次的
形态给空）：

| 场景 | 结果 |
|---|---|
| mock 模型按占位约定生成 | `best_effort=True, fallback_used=False, success=True`；报告显示代码 + 完整输出 + 「尽力而为」+ 「⚠️ 无法核对」 |
| 模型两次拒答 | `fallback_used=True, llm_calls=2`，落本地兜底脚本（1080 字符），真跑通、有输出、3 条假设入报告 |

两种场景的报告里都**没有**"未运行"、**没有**"复现状态: ✅/❌"。

**测试（646 → 661）**：`test_reproduction_core.py` 的
`TestInsufficientInfoShortCircuit` 重写为 `TestInsufficientInfoBestEffort`
（不再断言短路，改为断言真的生成并执行）；新增 `TestPlaceholderRecovery`
（定向重试预算 = 2、兜底脚本落盘并跑通、兜底脚本过全部门、兜底输出抽不出指标）
与 `TestValidatorBestEffort`（第四态、论文侧标记也足以判定、`not_runnable`
优先）；`test_code_generation_completeness.py` 新增 `TestReportSurfacesBestEffort`
（标注齐全、"未运行"不出现、语法门仍报未运行）与三条 prompt 用例（不足路径
不含占位标记、禁令仍在、充足路径逐字不变）；`test_orchestrator_storage.py`
新增跳过优化原因的区分用例。

**边界（本轮刻意不动）**：「信息充足但论文没声明数值」的论文仍走
`_local_compare` 的乐观判定（"跑出数值即算成功"）——那是既有的产品语义，
改它等于重定义"什么算复现成功"，另批处理。若在那类论文上看到 ✅，不是本次
改动带来的。

### 附带变更（Docker 沙箱有不可忽略的代价，改为显式开启）

用户此前那次跑通的运行走的是**本地隔离执行**：当时引擎没启动，
[2026.09.20-11] 的探测把开关拉回了关闭。引擎启动后开关默认打开，真实模式的
第一次运行会无声地落在容器里，而且每次都要在容器内现装依赖（tmpfs 随 `--rm`
清空，numpy 级别约 20 秒、torch 级别几分钟）——慢且难解释。

现改为 `use_docker` 默认 **False**（`app.py`）：真实模式默认走本地隔离执行
（依赖装进 `data/deps/<hash>/`，多论文共享去重），要用容器在侧边栏一键打开；
文案同步改口为「未启用容器沙箱，将使用本地隔离执行」/「将使用容器沙箱，
首跑会在容器内安装依赖，偏慢」。`tests/test_app_docker_gate.py` 的对应用例
改为断言「默认关 + 打开后真的改口」（缺一半都会让用户以为沙箱开着）。
...
- **信息不足不再编造代码**：`src/agents/code_executor.py::_info_insufficient`
  ...
  > ⚠️ 已被 [2026.09.20-14] 取代：信息不足**不再预先拒绝**，改为尽力而为
  > 生成最小可运行脚本并照常执行，结果带 `best_effort` 标注。禁令的宾语
  > 从「生成代码」改成「冒充结论」——不许把无关数据集写成论文结论，但不
  > 许用"信息不足"当交白卷的理由。
...
- **ResultValidator 区分「无法运行」与「复现失败」**：`src/agents/result_validator.py`
  - 新增三态 `status`：`not_runnable`（`is_reproduced=None`，代码没跑起来）/
    ...
    > ⚠️ 已被 [2026.09.20-14] 扩为**四态**：新增 `best_effort`（跑了，但论文
    > 信息不足、代码是占位实现——既不判成功也不判失败），优先级
    > `not_runnable` > `best_effort` > 正常比对。
...
- **报告如实渲染三态**：`src/agents/report_generator.py` 执行状态显示
  ...
  > ⚠️ [2026.09.20-14] 起「⚠️ 未运行」只留给两道真门（语法错误 / 危险调用），
  > 并新增「⚠️ 无法核对（论文信息不足）」这一验证状态。
